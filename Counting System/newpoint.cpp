...
int newpoint(const char * img1,const char * img2,std::vector<Movpts> * ADDR_inputPoints,std::vector<FeaturePoints> * ADDR_finalPoints)
{
...
//    if( srcA->step != srcB->step || velx->step != vely->step )
 //       CV_Error( CV_BadStep, "two source or two destination images have different steps" );
	...
	{
	//	cout<<liz<<endl;
		i = (*ADDR_inputPoints)[liz].y_pre;
		j = (*ADDR_inputPoints)[liz].x_pre;
	...
	//	cout<<"X1:"<<X1<<'\t'<<"Y1:"<<Y1<<endl;
		...
		if(abs(sumx)+abs(sumy)<50)//abs(sumx)+abs(sumy)>movjudge && 
		{
			tempPoint.x = j;
			tempPoint.y = i;
			(*ADDR_finalPoints).erase((*ADDR_finalPoints).end()-1,(*ADDR_finalPoints).end());
...
//	cvShowImage("After BM",output);
//	cvWaitKey(0);

	IplImage * image3 = cvLoadImage(img1);
	int num = (*ADDR_finalPoints).size();
	int lizdraw;
	int R = 0,
		G = 255,
		B = 0;
	...
	{
		centerpoint.x = (*ADDR_finalPoints)[lizdraw].x;
		centerpoint.y = (*ADDR_finalPoints)[lizdraw].y;
		cvCircle( image3, centerpoint ,3 , CV_RGB(R,G,B),1,  8, 3 );
		//cvSaveImage("p2.bmp",image3);
	}

	std::cout<<"New points number:"<<(*ADDR_finalPoints).size()<<std::endl;
